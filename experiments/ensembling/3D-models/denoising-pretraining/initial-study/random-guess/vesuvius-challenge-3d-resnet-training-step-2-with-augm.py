...
class VesuviusTrain(Dataset):
    ...
    def __getitem__(self, i):
        ...
        noise_vector = torch.randn_like(mask_crop) * noise_sigma
        ...
        return frag_crop, mask_crop, noise_vector
...
class VesuviusVal(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return frag_crop, mask_crop, noise_vector, torch.tensor([x1, y1, x2, y2], dtype=torch.int32)
...
class SegModel(nn.Module):
    ...
    def load_pretrained_weights(self, state_dict):
        ...
        # make all half float tensors float32
        for k, v in self.encoder.state_dict().items():
            self.encoder.state_dict()[k] = v.float()
...
x = torch.randn(5, 1, 20, 64, 64).to(device)
with torch.no_grad():
    y = model(x)
y.shape
...
gt_mask = torch.from_numpy(np.asarray(FRAGMENTS_ZARR[TEST_FRAGMENT].truth)).float().to(device)
...
for epoch in range(1, EPOCHS+1):
    ...
    for i, (fragments, masks, noise_vector) in pbar_train:
        fragments, masks, noise_vector = fragments.to(device), masks.to(device), noise_vector.to(device)
    ...
    final_pred_mask = torch.zeros(gt_shape, dtype=torch.float32, device=device)
    
    for i, (fragments, masks, noise_vector, xys) in pbar_val:
        fragments, masks, noise_vector = fragments.to(device), masks.to(device), noise_vector.to(device)
