...
class VesuviusTrain(Dataset):
    def __init__(self, fragments):
        ...
        for fragment in fragments:
            ...
            for y in tqdm(range(0, H-CROP_SIZE+1, TRAIN_STRIDE)):
                for x in range(0, W-CROP_SIZE+1, TRAIN_STRIDE):
                    ...
                    truth = FRAGMENTS_ZARR[fragment].truth[y:y+CROP_SIZE, x:x+CROP_SIZE]
                    if np.sum(truth>0.5) / (CROP_SIZE * CROP_SIZE) > 0.2:
                        self.xys.append((fragment, x, y, W, H))
        ...
        self.train_aug_list = [
                ...
                A.Rotate(limit=90, p=0.9), 
                # A.Affine(p=1.0, shear=10, rotate=90, scale=(0.5, 1), translate_percent=0.2),
    ...
    def __getitem__(self, i):
        ...
        mask_crop = FRAGMENTS_ZARR[fragment].truth[y1:y2, x1:x2]
        frag_crop = frag_crop/65535
        
        binary_mask_crop = FRAGMENTS_ZARR[fragment].mask[y1:y2, x1:x2]
        # print("tmp", frag_crop.min(), frag_crop.max(), binary_mask_crop.min(), binary_mask_crop.max())
        frag_crop = (1 - frag_crop) * np.expand_dims(binary_mask_crop, axis=-1)
        # print("tmp", frag_crop.min(), frag_crop.max(), binary_mask_crop.min(), binary_mask_crop.max())
        ...
        if np.random.uniform(0, 1) < 0.5:
            ...
            frag_crop_2 = FRAGMENTS_ZARR[fragment].surface_volume[y1:y2, x1:x2, 20+z_offset:40+z_offset]            
            mask_crop_2 = FRAGMENTS_ZARR[fragment].truth[y1:y2, x1:x2]
            frag_crop_2 = frag_crop_2/65535
            
            
            binary_mask_crop = FRAGMENTS_ZARR[fragment].truth[y1:y2, x1:x2]
            frag_crop_2 = (1 - frag_crop_2) * np.expand_dims(binary_mask_crop, axis=-1)
        ...
        # random number between 512 and 1024
        rand_wh = np.random.randint(512, 1024)
        rand_position_start_y = np.random.randint(0, CROP_SIZE-rand_wh)
        rand_position_start_x = np.random.randint(0, CROP_SIZE-rand_wh)
        
        frag_crop = frag_crop[rand_position_start_y:rand_position_start_y+rand_wh, rand_position_start_x:rand_position_start_x+rand_wh]
        mask_crop = mask_crop[rand_position_start_y:rand_position_start_y+rand_wh, rand_position_start_x:rand_position_start_x+rand_wh]
        ...
        frag_crop = frag_crop.unsqueeze(0).float()
...
class VesuviusVal(Dataset):
    def __init__(self, fragment):
        ...
        H, W = FRAGMENTS_SHAPE[fragment]
        for y in range(0, H-CROP_SIZE+1, VAL_TILE_SIZE):
            for x in range(0, W-CROP_SIZE+1, VAL_TILE_SIZE):
                self.xys.append((x, y))
    ...
    def __getitem__(self, i):
        ...
        binary_mask_crop = self.fragment.mask[y1:y2, x1:x2]
        frag_crop = (1 - frag_crop) * np.expand_dims(binary_mask_crop, axis=-1)
        # print(frag_crop.max(), frag_crop.min())
...
print(img.min(), img.max(), mask.shape, mask.min(), mask.max(), img.shape)
...
class SegModel(nn.Module):
    ...
    def forward(self, x):
        ...
        feat_maps_pooled = [torch.max(f, dim=2).values for f in feat_maps]
...
# x = torch.randn(5, 1, 20, 512, 512).cuda()
# with torch.no_grad():
#     y = model(x)
# y.shape
...
weights = [0.5, 2]  # Example of weights
class_weights = torch.FloatTensor(weights).cuda()
pos_weight = torch.tensor([1/6]).cuda()

def criterion(y_pred, y_true):
    # return FocalLoss(y_pred, y_true)
    
    return torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)(y_pred, y_true)
    # return BCELoss(y_pred, y_true) # + 0.5 * DiceLoss(y_pred, y_true)
...
for epoch in range(1, EPOCHS+1):
    ...
    for i, (fragments, masks) in pbar_train:
        ...
        with amp.autocast():
            ...
            loss = criterion(pred_masks.squeeze(), masks.squeeze())
    ...
    mask_count = torch.zeros(gt_shape, dtype=torch.float32, device='cuda')
    ...
    for i, (fragments, masks, xys) in pbar_val:
        ...
        with torch.no_grad():
            pred_masks = model(fragments)
            mloss_val += criterion(pred_masks.squeeze(), masks.squeeze()).item()
            pred_masks = torch.sigmoid(pred_masks)
            # upsample from 512x512 to 1024x1024
            pred_masks = F.interpolate(pred_masks, size=(1024, 1024), mode='bilinear', align_corners=False)
        ...
        for j, xy in enumerate(xys):
            ...
            mask_count[xy[1]:xy[3], xy[0]:xy[2]] += torch.ones((CROP_SIZE, CROP_SIZE)).cuda()


        pbar_val.set_description(("%10s") % (f"Val Loss: {mloss_val / (i+1):.4f}"))
        
    final_pred_mask /= mask_count
    
    out['loss_val'] = mloss_val / (i + 1)
    
    
    fbeta_best = 0
    for threshold in np.arange(0.1, 1.00, 0.01):
        fbeta = fbeta_score(final_pred_mask, gt_mask, threshold)
        print(f"Threshold : {threshold:.2f}\tFBeta : {fbeta:.6f}")
        
        
        if fbeta_best < fbeta:
            out[f"fbeta_best"] = fbeta_best
            out[f"threshold_best"] = threshold
            fbeta_best = fbeta.item()
