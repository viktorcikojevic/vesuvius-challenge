...
# load weights from 
model_path = "/home/viktor/Documents/kaggle/vesuvius-challenge/experiments/ensembling/3D-models/bce-dice/fold-1/depth-34/image-512-weighted-loss-v2/ckpts/resnet34_3d_seg_best_0.960.pt"
...
for k, v in state_dict.items():
    ...
    new_state_dict[name] = v.float()

model.load_state_dict(new_state_dict)
model_swa.load_state_dict(new_state_dict)
...
scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=0.001, max_lr=0.005, step_size_up=10, step_size_down=10)
...
for epoch in range(1, EPOCHS+1):
    ...
    # SWA update
    for param, param_swa in zip(model.parameters(), model_swa.parameters()):
        param_swa.data.mul_(n_swa).add_(param.data).div_(n_swa + 1)
    
    n_swa += 1
    
    
    # pass the swa_model through training data to update the batch norm statistics
    model_swa.train()
    pbar_train = enumerate(dataloader_train)
    # pbar_train = tqdm(pbar_train, total=n_train, bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}")
    pbar_train = tqdm(pbar_train, total=2*N_TRAIN, bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}")
    # make one pass to update the batch norm statistics for the swa_model
    for i, (fragments, masks) in pbar_train:
        fragments, masks = fragments.cuda(), masks.cuda()

        with torch.no_grad():
            pred_masks = model_swa(fragments)
        if i >= 2*N_TRAIN:
            break
    model_swa.eval()    
    ...
    fbeta_best = 0
    for threshold in np.arange(0.1, 1.00, 0.02):
        ...
        if fbeta_best < fbeta:
            fbeta_best = fbeta
            out[f"fbeta_best"] = fbeta.item()
            out["threshold"] = threshold
            get_ipython().system('rm ./ckpts/model_best_*')
    torch.save(model.state_dict(), f"./ckpts/model_epoch_{epoch}.pt")
    
    # ---- Validation for SWA model ----
    ...
    out['loss_val_swa'] = mloss_val / (i + 1)
    ...
    for threshold in np.arange(0.1, 1.00, 0.02):
        ...
        if fbeta_best < fbeta:
            fbeta_best = fbeta
            out[f"fbeta_best_swa"] = fbeta.item()
            out["threshold_swa"] = threshold
            get_ipython().system('rm ./ckpts/model_swa_best_*')
    torch.save(model.state_dict(), f"./ckpts/model_swa_epoch_{epoch}.pt")
