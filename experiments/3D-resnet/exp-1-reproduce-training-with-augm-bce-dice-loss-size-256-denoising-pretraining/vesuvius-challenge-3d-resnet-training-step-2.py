...
get_ipython().system('mkdir ./ckpts-2')
...
fragment_cropped = fragment.surface_volume[y:y+CROP_SIZE, x:x+CROP_SIZE, 0:40][:, :, ::2]
...
class VesuviusTrain(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # random_start = np.random.randint(-6, 0)
        # z1 += random_start
        # z2 += random_start
        ...
        if np.random.uniform(0, 1) < 0.5:
            frag_crop = FRAGMENTS_ZARR[fragment].surface_volume[y1:y2, x1:x2, 0:40][:, :, ::2]
        else:
            frag_crop = FRAGMENTS_ZARR[fragment].surface_volume[y1:y2, x1:x2, 1:41][:, :, ::2]
...
class VesuviusVal(Dataset):
    ...
    def __getitem__(self, i):
        ...
        frag_crop = self.fragment.surface_volume[y1:y2, x1:x2, 0:40][:, :, ::2]
...
x = torch.randn(5, 20, 256, 256).cuda()
with torch.no_grad():
    y = model(x)
y.shape
...
# BCELoss = smp.losses.SoftBCEWithLogitsLoss(smooth_factor=0.1, from_logits=False)
...
for epoch in range(1, EPOCHS+1):
    ...
    for threshold in np.arange(0.2, 1.00, 0.05):
        ...
        if fbeta_best < fbeta:
            fbeta_best = fbeta
            torch.save(model.state_dict(), f"./ckpts-2/resnet34_3d_seg_best_{fbeta_best:.4f}.pt")
    ...
    torch.save(model.state_dict(), f"./ckpts-2/resnet34_3d_seg_epoch_{epoch}.pt")
